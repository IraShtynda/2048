export default class GameService {
  constructor(storage) {
    ...
    this.bestScore = storage.get('2048') || 0;
  }
  ...
  moveBoard(board) {
    const newBoard = board.map(row => this.processRow(row));
    return newBoard;
  }
  ...
  canMove(board) {
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
        if (board[i][j].value === 0 || (j < board[j].length - 1 && board[i][j].value === board[i][j + 1].value)
  ...
  saveScore() {
    this.storage.set('2048', this.totalScore);
  }
}
